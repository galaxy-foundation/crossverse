		...
		if (wallet.connected) {
			if (status.quantity > wallet.balance) {
				...
				if (result.success && result.tx) {
					...
					if (res.status === 'ok') {
						res = (await call('/api/artwork/' + art.id, {action: 'listing'})) as ApiResponse
						if (res.status === 'ok') {
	...
	return (
		<Modal
			...
			footer={[
				...
				<Button key="submit" disabled={ wallet.checkingBalance || status.quantity > wallet.balance || status.quantity === 0 } type="primary" loading={wallet.checkingBalance || status.loading} onClick={onSubmit} style={{ fontSize: 'large', padding: '10px 30px', height: 'auto', display: !wallet.connected ? 'none' : '', }}>
					SUBMIT
				</Button>,
		...
		>
			...
			<div style={{ display: 'flex' }}>
				...
				<div style={{ flexGrow: 1, paddingLeft: 20 }}>
					<b>Quantity</b>
					<div>
						<Input type="number" value={status.quantity} min={1} max={wallet.balance} step={1} onChange={(e) => setQuantity(Math.min(wallet.balance, Number(e.target.value)))}/>
					</div>
					<b>Price</b>
					<div>
						<Input type="number" value={status.price} min={0.001} max={10000} step={0.0001} onChange={(e) => setPrice(Number(e.target.value))} />
					</div>
					<b>Amount</b>
